...
## 🌬️ Wind Turbine Module (NEW)

### Problem it solves
Wind turbines fail silently — gearbox overheating, blade imbalance, and bearing wear cost operators **$150K–$500K per unplanned shutdown**. This module monitors real-time sensor data and predicts failures **before** they happen.

### What it does
| Feature | Description |
|---|---|
| **Rotor Efficiency Prediction** | ML model (R² = 0.96) predicts current efficiency from 11 sensor inputs |
| **Component Health Monitoring** | Real-time status of gearbox, generator, bearing temperatures |
| **Failure Risk Score** | 0–100% risk score with actionable maintenance recommendations |
| **Site Suitability Assessment** | Classifies whether a location is suitable for wind turbine installation (99.3% accuracy) |
| **Power Output Estimation** | Estimates actual kW output, daily kWh, and annual MWh based on conditions |
| **CO₂ Avoidance Tracking** | Shows tonnes of CO₂ offset per year vs grid average |
| **3D Digital Twin** | Interactive Three.js turbine with live blade rotation speed and vibration-driven alerts |
| **Map Integration** | Click any location on the map to auto-fill wind speed + direction from Open-Meteo API |
| **AI Maintenance Insights** | Rule-based (or OpenAI-powered) component diagnosis with specific repair guidance |

### Running the Wind Module
```bash
# Step 1: Generate wind models (one-time)
cd ml/src
python gen_wind_models.py

# Step 2: Start the server (same as before — both modules run together)
python app.py

# Step 3: Open dashboards
http://localhost:5001       ← Solar panel dashboard
http://localhost:5001/wind  ← Wind turbine dashboard
```
